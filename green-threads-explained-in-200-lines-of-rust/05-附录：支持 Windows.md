...
我们的示例适用于OSX，Linux和Windows，但正如我所指出的那样，即使Windows实施可行，也不正确。由于我一直致力于在所有三个平台上都可以做到这一点，因此我将在本章中介绍我们需要做的事情。
您可能想知道为什么我没有在原始代码中包含此内容，其原因是，这实际上与解释我想探索的主要概念没有任何关系。
在这里，我试图进一步探讨如何为Windows正确设置堆栈并进行适当的上下文切换。即使我们可能无法一路实现完美的实现，也有大量信息和参考供您进一步探索，我将在这里列出其中的一些：

* Microsoft x64 软件约定
* Win64 / AMD64 API-psABi与Win64之间差异的简要概述
* 适用于Windows的手工协程-关于协程实现的很好阅读
* Boost Context程序集-在C ++中，但它是进一步研究的良好参考

### Windows 的特殊之处

我认为在主要示例中实现此重要性不够的原因是，除了我们需要的一个文献记载很少的怪癖之外，Windows还拥有更多的“被调用者保存”寄存器或调用它们的“非易失性”寄存器。考虑到这一点，所以我们真正要做的只是在执行上下文切换时保存更多数据，而这需要更多的条件编译。

> 有条件地对其进行编译以支持Windows可以使我们的代码几乎膨胀50％，而又不会增加基本理解所需的内容。
相反，这并不意味着没有意思，但是从头开始做所有事情时，我们还将亲身体验支持多个平台的一些困难。

### 额外的由被调用方保存的（非易失性）寄存器

我提到的第一件事是 Windows 希望在上下文切换期间保存更多数据，尤其是 XMM6-XMM15 寄存器。 实际上在参考中特别提到了它，因此这只是向我们的ThreadContext结构中添加了更多字段。 既然我们之前做过一次，这很容易。
除了XMM 寄存器，rdi 和 rsi 寄存器在 Windows 上也是非易失性的，这意味着它们已被调用者保存（在Linux上，这些寄存器用于第一个和第二个函数参数），因此我们也需要添加它们。
但是，有一个警告：`XMM` 寄存器是 128 位，而不是 64 位。Rust 具有 `u128` 类型，但是我们将使用 `[u64; 2]`来避免可能导致的某些对齐问题。 不用担心，我会在下面进一步解释。
现在，我们的ThreadContext如下所示：
